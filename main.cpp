...
hittable_list random_scene() {
    ...
    world.add(make_shared<sphere>(vec3(0, -1000, 0), 1000, make_shared<lambertian_>(checker)));
                ...
                if (choose_mat < 0.8) {
                    ...
                    world.add(
                        make_shared<moving_sphere>(
                            ...
                            make_shared<lambertian_>(albedo)));
                }
    ...
    world.add(
        make_shared<sphere>(vec3(-4, 1, 0), 1.0, make_shared<lambertian_>(vec3(0.4, 0.2, 0.1))));

    world.add(
        make_shared<sphere>(vec3(4, 1, 0), 1.0, make_shared<metal>(vec3(0.7, 0.6, 0.5), 0.3)));
...
hittable_list two_spheres() {
    ...
    objects.add(make_shared<sphere>(vec3(0, -10, 0), 10, make_shared<lambertian_>(checker)));
    objects.add(make_shared<sphere>(vec3(0, 10, 0), 10, make_shared<lambertian_>(checker)));
...
hittable_list two_perlin_spheres() {
    ...
    objects.add(make_shared<sphere>(vec3(0, -1000, 0), 1000, make_shared<lambertian_>(pertext)));
    objects.add(make_shared<sphere>(vec3(0, 2, 0), 2, make_shared<lambertian_>(pertext)));
...
hittable_list earth() {
    ...
    auto earth_surface =
        make_shared<lambertian_>(make_shared<image_texture>(texture_data, nx, ny));
...
hittable_list cornell_box() {
    ...
    auto red = make_shared<lambertian_>(make_shared<constant_texture>(vec3(0.65, 0.05, 0.05)));
    auto white = make_shared<lambertian_>(make_shared<constant_texture>(vec3(0.73, 0.73, 0.73)));
    auto green = make_shared<lambertian_>(make_shared<constant_texture>(vec3(0.12, 0.45, 0.15)));
...
hittable_list final_scene() {
    ...
    auto ground =
        make_shared<lambertian_>(make_shared<constant_texture>(vec3(0.48, 0.83, 0.53)));
    ...
    auto moving_sphere_material =
        make_shared<lambertian_>(make_shared<constant_texture>(vec3(0.7, 0.3, 0.1)));
    ...
    auto emat = make_shared<lambertian_>(make_shared<image_texture>(tex_data, nx, ny));
    objects.add(make_shared<sphere>(vec3(400, 200, 400), 100, emat));
    auto pertext = make_shared<noise_texture>(0.1);
    objects.add(make_shared<sphere>(vec3(220, 280, 300), 80, make_shared<lambertian_>(pertext)));
